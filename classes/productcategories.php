...
class ProductCategory {
	...
	function set_category($category_id) { 
		global $db;
		$query = 'SELECT * from product_categories WHERE store_id="'.$_SESSION['store_id'].'" AND category_id="'.$category_id.'"';
	...
	function update_category($category_id, $category_name, $category_description) { 
		...
		$query = 'UPDATE product_categories SET
				  ...
				  category_description = "'.$category_description.'"
				   WHERE category_id="'.$category_id.'" AND store_id="'.$_SESSION['store_id'].'"';
		$result = $db->query($query) or die($db->error);
	...
	function add_category($category_name, $category_description) { 
		...
		$query = "SELECT * from product_categories WHERE store_id='".$_SESSION['store_id']."' AND category_name='".$category_name."'";
		$result = $db->query($query) or die($db->error);
		...
		if($num_rows > 0) { 
			...
			$query = "INSERT into product_categories VALUES(NULL, '".$category_name."', '".$category_description."', '".$_SESSION['store_id']."')";
			$result = $db->query($query) or die($db->error);
	...
	function list_categories() {
		global $db;
			$query = 'SELECT * from product_categories WHERE  store_id="'.$_SESSION['store_id'].'" ORDER by category_name ASC';
			$result = $db->query($query) or die($db->error);
			$content = '';
			$count = 0;
			while($row = $result->fetch_array()) { 
				...
				$content .= '<td>';
				$content .= '<form method="post" name="edit" action="manage_categories.php">';
