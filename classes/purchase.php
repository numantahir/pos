...
class Purchase { 
	...
	function list_periodical_purchases($start_date, $end_date) { 
		global $db;
		
		$from = $start_date;
		$to = $end_date;
		
		$query = "SELECT * from purchases WHERE store_id='".$_SESSION['store_id']."' AND datetime between '".$from."' AND '".$to."' ORDER by purchase_id DESC";
		$result = $db->query($query) or die($db->error);
