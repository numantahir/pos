        	...
        	<table width="100%" cellpadding="10px" border="0px">
            	<tr>
                    ...
                    <td style="text-align:right;">
                    	<h1 style="color:#CCC;">Sale Return Invoice</h1>
                        <?php $mysqldate = strtotime($sale_rt->get_sale_return_info($_GET['sale_rt_id'], 'datetime')); ?>
                        <p>Date: <?php echo date('d-M-Y', $mysqldate); ?><br />
                        S.INV # : <?php echo $_GET['sale_rt_id']; ?><br />
                        <?php $agent_id = $sale_rt->get_sale_return_info($_GET['sale_rt_id'], 'agent_id'); ?>
                        Agent: <?php echo $user->get_user_info($agent_id, 'first_name').' '.$user->get_user_info($agent_id, 'last_name'); ?><br>
						Manual Inv#: <?php echo $sale_rt->get_sale_return_info($_GET['sale_rt_id'], 'invoice_no'); ?><br>
						Payment Type: <?php echo $sale_rt->get_sale_return_info($_GET['sale_rt_id'], 'payment_status'); ?>
                        </p>
...
<table width="45%" border="1px" cellspacing="0" cellpadding="5px">
    ...
    <tr>
    	<td>
        <?php $client_id = $sale_rt->get_sale_return_info($_GET['sale_rt_id'], 'client_id'); ?>
...
<?php $invoice_detail = $sale_rt->view_sale_return_invoice($_GET['sale_rt_id']); ?>
...
<table width="100%" cellpadding="5px" cellspacing="0" align="right">
	<tr>
    	<td>
            ...
            <div style="width:450px; min-height:70px; border:1px solid #000; padding:5px;"><?php echo $sale_rt->get_sale_return_info($_GET['sale_rt_id'], 'memo'); ?></div>
